...
def update_cache(cache: dict, stock_code: str, new_data: dict) -> dict:
    ...
    参数:
        cache: 现有缓存
        stock_code: 股票代码
        new_data: 新数据 {"daily_changes": [], "dates": []}
    ...
    unique_dates = []
    unique_changes = []
    for d, c in zip(reversed(all_dates), reversed(all_changes)):
        if d not in seen:
            seen.add(d)
            unique_dates.insert(0, d)
            unique_changes.insert(0, c)
...
def read_stock_list_from_md(file_path: str) -> list:
    ...
    返回:
        分组后的股票列表，每个元素为 (分组名, [股票名列表])
        如果文件没有分组，返回 [(None, [所有股票])]
    """
    groups = []
    current_group = None
    current_stocks = []
    ...
    with open(file_path, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            if line.startswith('## '):
                ...
                current_stocks.append(clean_name)
...
def get_stock_data(stock_code: str, cache: dict = None, max_retries: int = 3) -> dict:
    ...
    for retry in range(max_retries):
        try:
            ...
            # 提取日期和涨跌幅（volumes 必须在 dropna 后提取，保持一致）
...
def main():
    ...
    # 定义单只股票的处理函数（用于线程池）
    def process_single_stock(stock_name):
    ...
    for group_name, stock_names in groups:
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            # 提交所有任务
            future_to_stock = {
                executor.submit(process_single_stock, stock_name): stock_name 
    ...
    generate_md_report(grouped_results, output_path, input_path)
