...
### 试错 16：线程无超时导致整流程卡死
- **尝试**：使用 `ThreadPoolExecutor` + `as_completed` 并行获取数据
- **结果**：单只股票 API 卡住时，`as_completed` 无限等待，整个脚本无法继续
- **结论**：`future.result(timeout=30)` 增加超时，超时后标记为失败并继续

### 试错 17：北交所股票代码无前缀导致 API 报错
- **尝试**：`extract_stock_code_from_parentheses` 只处理 6/0/3 开头代码
- **结果**：惠丰钻石（920725）被映射为无前缀的 "920725"，akshare 报 `invalid literal for int()`
- **结论**：增加北交所代码识别（4/8/92 开头 → `bj` 前缀），并在 API 调用前跳过

### 试错 18：缺失数据需 akshare 实时获取
...
### 决策 13：看板数据从内嵌改为 API 动态加载
- **决策**：`generate_dashboard.py` 不再内嵌数据到 HTML，改为写入 `.index_data/` 数据文件 + 生成瘦身 HTML，前端通过 `/api/data/*` 异步加载。
- **理由**：
  - 旧方案 HTML ~1MB（浏览器卡顿），需 `_unbundle_dashboard.py` 后处理提取数据
  - 新方案 HTML ~55KB，一条命令直接产出数据文件 + HTML，消除中间步骤
  - 数据文件可被 `dashboard_server.py` 直接服务，前后端解耦
- **约束**：必须在 `generate_dashboard.py` 模板中修改前端代码，禁止直接修改 `dashboard.html`（会被每次生成覆盖）

### 决策 14：MA5 排名趋势独立为新 Tab
- **决策**：新增「5日线角度」Tab，基于 `_gen_ma5_trend.py` 生成的 `ma5_trend.json` 展示近5日排名变化。
- **理由**：
  - 用户明确需要排名变化趋势可视化，两日对比绿+升/橙-降
  - 题材标签筛选（top3 平均排名）便于按板块审视排名变化
  - `_gen_ma5_trend.py` 修复多个数据质量问题（CSV中英文表头、代码前缀不一致、全角半角不统一）
- **约束**：仅取前 200 名；新进标的用 `200 - 排名` 计算增幅
...
- **理由**：
  - 数据源从 iframe 3（56 个涨停+1个炸板，异动原因粗略）切换到 iframe 7 全部展开视图（66 只封板，异动原因详细，含板形/换手率/龙虎榜等 16 列）
  - 手工浏览器操作不稳定，脚本可复现；匹配验证（格式/去重/板块唯一/单次出现）内建，避免手动 7 步检查
  - 模型审查发现的关键词遗漏（太空算力→商业航天、特种气体→半导体材料、推理服务器→AI 应用）写入代码层，下次自动匹配，形成持续收敛的闭环
- **约束**：依赖 agent-browser daemon；iframe 索引（`[7]`）若页面改版需手动调整
